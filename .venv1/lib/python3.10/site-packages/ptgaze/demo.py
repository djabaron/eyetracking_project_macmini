...
class Demo:
    ...
    # true when pressing the space key (for showing the logger.info)
    space_key = False
    ...
    def __init__(self, config: DictConfig):
        ...
        self.last_look_time = None
        self.look_grace_period = 0.5
    ...
    def _create_capture(self) -> Optional[cv2.VideoCapture]:
        if self.config.demo.image_path:
            return None
        if self.config.demo.use_camera:
            # For the Laptop Webcam use cv2.VideoCapture(0)
            # For an external Webcam (USB) use cv2.VideoCapture(1)
            cam_index = self.config.demo.get("camera_id", 0)
            cap = cv2.VideoCapture(cam_index)  # force DShow backend on Windows
    ...
    def _draw_gaze_vector(self, face: Face) -> None:
        ...
        if self.config.mode == 'MPIIGaze':
            ...
            # Looking at the cam:
            #   Webcam: Pitch -11, Yaw 3
            #   external Cam USB: Pitch -16, Yaw 0
            #   Problem: Yaw varies if you're not sitting right in front of the cam
            if -20 < pitch < -12 and abs(yaw) < 20:
                if not self.is_looking:
                    ...
                    duration = time.time() - self.start_look_time
                    if duration >= 3:
                        text = "3 sec +"
                        color = (0, 255, 0)
                    else:
            ...
            elif self.is_looking and self.last_look_time and time.time()-self.last_look_time<self.look_grace_period:
                duration = time.time() - self.start_look_time
                if duration >= 3:
                    text = "3 sec +"
                else:
                    text = f"LOOKING ({duration:.1f}s)"
                color = (0, 255, 0)
            ...
            cv2.putText(self.visualizer.image, text, (20, 40),
